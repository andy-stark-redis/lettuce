    ...
    public void run() {
        ...
        try (StatefulRedisConnection<String, String> connection = redisClient.connect();
            ...
            CompletableFuture<Void> setup = async.setbit("A", 0, 1).thenCompose(v -> async.setbit("A", 1, 1))
                    ...
                    .thenCompose(v -> async.setbit("C", 5, 1))
                    .thenCompose(v -> asyncBytes.get("A".getBytes()))
                    .thenApply(res -> {
                    ...
                    .thenCompose(v -> asyncBytes.get("B".getBytes()))
                    .thenApply(res -> {
                        byte b = (res != null && res.length > 0) ? res[0] : 0;
                    ...
                    .thenCompose(v -> asyncBytes.get("C".getBytes()))
                    .thenAccept(res -> {
                        byte b = (res != null && res.length > 0) ? res[0] : 0;
            ...
            CompletableFuture<Void> andOp = async.bitopAnd("R", "A", "B", "C")
                    .thenCompose(len -> asyncBytes.get("R".getBytes()))
                    .thenAccept(res -> {
                        byte b = (res != null && res.length > 0) ? res[0] : 0;
            ...
            // STEP_START bitop_or
            CompletableFuture<Void> orOp = async.bitopOr("R", "A", "B", "C")
                    .thenCompose(len -> asyncBytes.get("R".getBytes()))
                    .thenAccept(res -> {
                        byte b = (res != null && res.length > 0) ? res[0] : 0;
                        String bits = String.format("%8s", Integer.toBinaryString(b & 0xFF)).replace(' ', '0');
            ...
            CompletableFuture<Void> xorOp = async.bitopXor("R", "A", "B")
                    .thenCompose(len -> asyncBytes.get("R".getBytes()))
                    .thenAccept(res -> {
                        byte b = (res != null && res.length > 0) ? res[0] : 0;
                        String bits = String.format("%8s", Integer.toBinaryString(b & 0xFF)).replace(' ', '0');
            ...
            CompletableFuture<Void> notOp = async.bitopNot("R", "A")
                    .thenCompose(len -> asyncBytes.get("R".getBytes()))
                    .thenAccept(res -> {
                        byte b = (res != null && res.length > 0) ? res[0] : 0;
                        String bits = String.format("%8s", Integer.toBinaryString(b & 0xFF)).replace(' ', '0');
